...
async function displayCheckInDate() {
  ...
  const checkInDate = attendance.checkInTime
    ? format(
        ...
        //       "Asia/Jakarta"
        //   ),
        toZonedTime(new Date(Number(attendance.checkInTime)), "Asia/Jakarta"),
        "HH:mm"
      )
    : "--:--";
...
async function displayCheckOutDate() {
  ...
  const checkOutDate = attendance.checkOutTime
    ? format(
        // new TZDate(new Date(Number(attendance.checkOutTime)), "Asia/Jakarta"),
        toZonedTime(new Date(Number(attendance.checkOutTime)), "Asia/Jakarta"),
        "HH:mm"
      )
    : "--:--";
        ...
        <div className="relative block overflow-hidden rounded-lg bg-base-100 border-gray-100 p-4 sm:p-6 lg:p-8 shadow-lg">
          ...
          <div className="sm:flex sm:justify-between sm:gap-4">
            <div>
              <h2>
                Today [{" "}
                {format(new TZDate(new Date(today)), "EEEE, MMM d, yyyy")}]
              </h2>
            </div>
          </div>
